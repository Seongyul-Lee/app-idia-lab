...
|--------|--------|--------|--------|--------|
| **profiles** | 본인 + 타인 닉네임/레벨 (제한 필드) | 자동 (auth trigger) | 본인만 | 불가 (soft delete) |
...
CREATE POLICY "profiles_select_public" ON public.profiles
  ...
  -- 실제 앱에서는 별도의 public_profiles 뷰 사용 권장
...
BEGIN
  ...
  IF p_is_no_spend THEN
    SELECT COUNT(*)::INTEGER INTO v_new_streak
    FROM (
      SELECT record_date
    ...
    WHERE record_date = CURRENT_DATE - (ROW_NUMBER() OVER (ORDER BY record_date DESC) - 1)::INTEGER;

    -- 위 서브쿼리가 동작하지 않을 수 있으므로 반복문으로 안전하게 계산
    v_new_streak := 0;
    FOR v_prev_date IN
...
BEGIN
  ...
  LOOP
    ...
    -- 기간 만료 판정
    IF CURRENT_DATE > v_participant.end_date AND v_participant.status = 'active' THEN
...
| 6 | 핵심 DB Function SQL 본문 완성 | PASS | 6개 RPC 모두 완전한 SQL 작성 (스텁 아님) |
